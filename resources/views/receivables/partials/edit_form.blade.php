<form id="receivableEditForm" method="POST" action="{{ route('receivables.update', $receivable->id) }}" class="space-y-6">
    @csrf
    @method('PUT')

    <!-- Header Section -->
    <div class="text-center border-b border-gray-200 pb-4">
        <h3 class="text-lg font-semibold text-gray-900">تعديل المستحق</h3>
        <p class="text-sm text-gray-600 mt-1">تعديل بيانات المستحق</p>
    </div>
    ...
    <div class="space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="space-y-2">
                ...
                <input name="source" type="text" required value="{{ $receivable->source }}"
            ...
            <div class="space-y-2">
                ...
                <input name="amount" type="number" step="0.01" required value="{{ $receivable->amount }}"
                       class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-all duration-200 placeholder-gray-400"
                       placeholder="0.00" />
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="space-y-2">
                ...
                <input name="date" type="date" required value="{{ $receivable->date }}"
            ...
            <div class="space-y-2">
                ...
                <select name="status" required id="status-select"
                    ...
                    <option value="paid" class="option-paid" {{ $receivable->status == 'paid' ? 'selected' : '' }}>🟢 مدفوعة</option>
                    <option value="unpaid" class="option-unpaid" {{ $receivable->status == 'unpaid' ? 'selected' : '' }}>🔴 غير مدفوعة</option>
                    <option value="partial" class="option-partial" {{ $receivable->status == 'partial' ? 'selected' : '' }}>🟡 مدفوعة جزئياً</option>
                </select>
            </div>
        </div>
        ...
        <div class="space-y-2">
            ...
            <input name="description" type="text" value="{{ $receivable->description }}"
                   class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-all duration-200 placeholder-gray-400"
        ...
        <div class="space-y-2" id="remaining-amount-display">
            ...
            <div class="relative">
                <input name="remaining_amount" type="number" step="0.01" id="remaining-amount-input" value="{{ $receivable->remaining_amount }}"
        ...
        <div class="space-y-2">
            ...
            <textarea name="notes" rows="3"
                      ...
                      placeholder="أضف أي ملاحظات إضافية...">{{ $receivable->notes }}</textarea>
        </div>
    </div>
    ...
    <div class="flex items-center justify-end pt-4 border-t border-gray-200">
        <button type="submit"
                class="px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition-all duration-200 font-medium shadow-sm hover:shadow-md">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const form = document.getElementById('receivableEditForm');
    if (form) {
